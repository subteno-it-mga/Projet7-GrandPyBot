...
def get_quote():
    ...
    """ Test if the function is not None
    >>> type(get_quote())
    <class 'str'>
    """
...
def ask_wiki(research_story):
    ...
    final_second_url = url_wiki + action + \
        final_format + "&pageids=" + str(page_id) + param
...
def data_treatment(parse, gmap_get_json, api_key):
    ...
    try:
        ...
        get_story_final = random_sentence + \
            get_story[0]+"<a target='_blank' href='http://fr.wikipedia.org/?curid=%s'>EN SAVOIR PLUS SUR WIKIPEDIA</a>" % (
                get_story[1]['pageid'])
